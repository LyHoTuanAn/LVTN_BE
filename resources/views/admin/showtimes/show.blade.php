    ...
    <div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="display: flex; gap: 10px;">
            <a 
                ...
                style="padding: 8px 20px; background: #9b59b6; color: white; text-decoration: none; border-radius: 6px; font-size: 0.9em;"
            >
                🎫 {{ __('Seat Map') }}
            </a>
            <a 
                ...
                <tbody>
                    @foreach ($showtime->bookings as $booking)
                        <tr style="border-bottom: 1px solid #dee2e6;">
                            <td style="padding: 12px; font-weight: 500;">{{ $booking->code }}</td>
                            <td style="padding: 12px;">{{ $booking->user?->name ?? '-' }}</td>
                            <td style="padding: 12px;">{{ number_format($booking->total_price, 0, ',', '.') }} VNĐ</td>
