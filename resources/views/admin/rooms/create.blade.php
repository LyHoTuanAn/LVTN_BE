            ...
            <div style="margin-bottom: 20px;">
                ...
                >
                    ...
                    @foreach ($cinemas as $cinema)
                        <option value="{{ $cinema->id }}" {{ old('cinema_id', $selectedCinemaId) == $cinema->id ? 'selected' : '' }}>
                ...
                @if($cinemas->isEmpty())
                    <p style="color: #f39c12; font-size: 0.85em; margin-top: 8px;">
                        {{ __('No cinemas available.') }} 
                        <a href="{{ route('admin.cinemas.create') }}" style="color: #3498db;">{{ __('Create one first') }}</a>
                    </p>
                @endif
            </div>
